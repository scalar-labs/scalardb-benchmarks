...
public class VtLoadRunner {
  private static final Logger LOGGER = LoggerFactory.getLogger(VtLoadRunner.class);
  ...
  public VtLoadRunner(Config config, DistributedStorage storage, int threadId) {
    this.id = threadId;
  ...
  private void putForSingleStorage(DistributedStorage storage, int userId, String payload)
    ...
    Put put = preparePut(YcsbCommon.NAMESPACE, YcsbCommon.TABLE + "_vt", userId, payload);
    storage.put(put);
  }
