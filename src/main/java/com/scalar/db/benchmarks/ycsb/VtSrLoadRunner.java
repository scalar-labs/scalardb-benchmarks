...
public class VtSrLoadRunner {
  private static final Logger LOGGER = LoggerFactory.getLogger(VtSrLoadRunner.class);
  ...
  public VtSrLoadRunner(Config config, DistributedTransactionManager manager, int threadId) {
    this.id = threadId;
    this.manager = manager;
    concurrency = getLoadConcurrency(config);
    batchSize = getLoadBatchSize(config);
    recordCount = getRecordCount(config);
    payload = new char[getPayloadSize(config)/2];
  ...
  private void putForSingleStorage(DistributedTransaction transaction, int userId, String payload)
    ...
    Put put = preparePut(YcsbCommon.NAMESPACE, YcsbCommon.TABLE + "_vtsr_app", userId, payload);
    transaction.put(put);
    put = preparePut(YcsbCommon.NAMESPACE, YcsbCommon.TABLE + "_vtsr_meta", userId, payload);
    transaction.put(put);
  }
