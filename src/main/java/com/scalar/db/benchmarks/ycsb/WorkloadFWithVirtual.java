...
public class WorkloadFWithVirtual extends TimeBasedProcessor {
  ...
  public WorkloadFWithVirtual(Config config) {
    super(config);
    ...
    while (true) {
      ...
      try {
        for (int i = 0; i < userIds.size(); i++) {
          ...
          transaction.get(prepareGet(YcsbCommon.NAMESPACE, YcsbCommon.TABLE + "_vt_scalardb", userId));
          transaction.put(preparePut(YcsbCommon.NAMESPACE, YcsbCommon.TABLE + "_vt_scalardb", userId, payloads.get(i)));
        }
        transaction.commit();
        break;
